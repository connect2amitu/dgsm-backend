...
if(isset($data)){
    $formData=$data[0];
    $redirect='artists/edit/'.$formData['id'];
    $btnName="Update";
}else{
...
<form class="form-horizontal form-validate-jquery" method="POST" action="<?= base_url($redirect); ?>">
  <div class="panel panel-flat">
    <div class="panel-heading">
      <h5 class="panel-title"><?= $btnName ?> artist</h5>
    </div>
    ...
    <div class="panel-body">
          ...
          <div class="form-group">
            ...
            <div class="col-lg-9">
              <input autocomplete="off" autofocus name="name" required="required" value="<?= $formData ? $formData['name']:"" ?>" type="text"
                class="form-control" placeholder="Enter artist Name">
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="text-right">
        ...
        <a href="<?= base_url('artists'); ?>" class="btn btn-danger access-multiple-open legitRipple"><i
...
<script>
    ...
    var validator = $(".form-validate-jquery").validate({
        ...
        messages: {
            name:"Name should greater than 3 characters",
        }
    });
